...
from src.utils.jwt_helper import jwt_required, get_current_user_id, get_current_user_role
...
def create_bonus_policy():
    ...
    try:
        ...
        policy = BonusPolicy(
            ...
            target_positions=json.dumps(data.get('target_positions', [])),
...
def update_bonus_policy(policy_id):
    ...
    try:
        ...
        if 'target_departments' in data:
            policy.target_departments = json.dumps(data['target_departments'])
        if 'target_positions' in data:
            policy.target_positions = json.dumps(data['target_positions'])
