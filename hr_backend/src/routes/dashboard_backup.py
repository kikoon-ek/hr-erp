...
def get_dashboard_overview():
    ...
    try:
        ...
        attendance_stats = db.session.query(
            ...
            func.sum(func.case((AttendanceRecord.status == '지각', 1), else_=0)).label('late_count'),
            func.sum(func.case((AttendanceRecord.status == '결근', 1), else_=0)).label('absent_count')
        ).filter(
        ...
        evaluation_stats = db.session.query(
            ...
            func.sum(func.case((Evaluation.status == 'completed', 1), else_=0)).label('completed_evaluations'),
...
def get_attendance_trend():
    ...
    try:
        ...
        monthly_stats = db.session.query(
            ...
            func.count(AttendanceRecord.id).label('total_records'),
            func.sum(func.case((AttendanceRecord.status == '출근', 1), else_=0)).label('on_time'),
            func.sum(func.case((AttendanceRecord.status == '지각', 1), else_=0)).label('late'),
            func.sum(func.case((AttendanceRecord.status == '결근', 1), else_=0)).label('absent'),
...
def get_summary_report():
    ...
    try:
        ...
        attendance_summary = db.session.query(
            ...
            func.avg(AttendanceRecord.work_hours).label('avg_hours'),
            func.sum(func.case((AttendanceRecord.status == '지각', 1), else_=0)).label('late_days'),
            func.sum(func.case((AttendanceRecord.status == '결근', 1), else_=0)).label('absent_days')
        ...
        evaluation_summary = db.session.query(
            func.count(Evaluation.id).label('total_evaluations'),
            func.sum(func.case((Evaluation.status == 'completed', 1), else_=0)).label('completed'),
            func.avg(func.case((Evaluation.status == 'completed', Evaluation.total_score), else_=None)).label('avg_score')
